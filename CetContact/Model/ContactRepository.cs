...
namespace CetContact.Model
{
    public class ContactRepository
    {
        ...
        public async Task<List<ContactInfo>> SearchContacts(string keyword)
        {
            keyword = keyword.ToLower();
            return await database.Table<ContactInfo>()
                                .Where(c => c.Name.ToLower().Contains(keyword) || c.Email.ToLower().Contains(keyword))
                                .ToListAsync();
        }
